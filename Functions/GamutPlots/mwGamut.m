function [lastContrast, RGBstoppedAt, flagsOutOfGamut, unclampedRGB] = mwGamut(AxisScale, luminance, varargin)
...
%% Parse
p = inputParser;
addRequired(p, 'AxisScale', @isnumeric);
...
parse(p, AxisScale, luminance, varargin{:})
...
endContrast = 150; % Optional, to avoid infinite loop. Set to -1 to run forever, if you're really sure
...
luminance = repmat(luminance, size(unscaledLM));
...
%     end
    ...
    MB = mw2mb(MW, 'AxisScale', [AxisScale AxisScale], 'MBScale', [1 1 1]);
    LMS = mb2lms(MB);
    LMS = MB(:,1:3);
    [RGB, sat, orig] = lms2rgb(LMS);
